...
.text  
main:  jal  TRACK		# draw trackline
	addi  $a0, $zero, 90	# Marsbot rotates 90* and start running
	jal   ROTATE
	jal   GO
sleep0: addi    $v0,$zero,32    # Keep running by sleeping in1000 ms
	li  $a0,8000        
	syscall
	jal     UNTRACK		# keep old track
	#jal     TRACK		# and draw new track line
	
	#jal  TRACK		# draw trackline
	addi  $a0, $zero, 180	# Marsbot rotates 180* and start running
	jal   ROTATE
	jal   GO
sleep1: addi    $v0,$zero,32    # Keep running by sleeping in3000 ms
	li  $a0,3000        
	syscall
	jal     UNTRACK		# keep old track
	jal     TRACK		# and draw new track line
	
goASKEWRIGHT: addi    $a0, $zero, 150	# Marsbot rotates 150*
	jal     ROTATE
sleep2: addi $v0,$zero,32  # Keep running by sleeping in 5000 ms
	...
	jal     UNTRACK         # keep old track
	jal     TRACK           # and draw new track line
goLEFT: addi  $a0, $zero, 270	# Marsbotrotates 270*  
	jal   ROTATE
...
goASKEWLEFT:addi  $a0, $zero, 30	# Marsbot rotates 30*  
	jal   ROTATE    
